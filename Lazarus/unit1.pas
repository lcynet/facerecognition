...
uses
  ...
  , Types, LCLType, LCLProc, LCLIntf, IntfGraphics, GraphType
  , FaceRecognition
  ;
...
type
  ...
  private
    procedure ConvertBitmap32bitTo24bit(bmp: TBitmap);
  public
...
procedure TForm1.ConvertBitmap32bitTo24bit(bmp: TBitmap);
var
  Source: TLazIntfImage;
  Dest: TLazIntfImage;
  ImgFormatDescription: TRawImageDescription;
begin
  Source := bmp.CreateIntfImage;
  Dest := TLazIntfImage.Create(bmp.Width, bmp.Height);
  try
    ImgFormatDescription.Init_BPP24_B8G8R8_BIO_TTB(bmp.Width, bmp.Height);
    Dest.DataDescription:=ImgFormatDescription;
    Dest.CopyPixels(Source);
    bmp.PixelFormat:=pf24bit;
    bmp.LoadFromIntfImage(Dest);
  finally
    Dest.Free;
    Source.Free;
  end;
...
begin
  ...
  try
    ...
    ConvertBitmap32bitTo24bit( bmp_temp ); //convert 32bit to 24bit bitmap, otherwise we got some error issues
  ...
  //draw white retangle on detected faces
  for I := 0 to Length(face_locations) - 1 do
  begin
  ...
  begin
    compare_face( face_encodings[0], face_encodings[i], Distance ); // <------ you can choose another Source value like face_encodings[1] or face_encodings[5] for Bruce Wills and see what happens ;-)
...
begin
  ...
  for i := 0 to Length(face_encodings) - 1 do
  begin
    compare_face(face_encodings[0], face_encodings[i], Distance); // <------ you can choose another source value than "face_encodings[0]" like "face_encodings[1]" or "face_encodings[5]" for Bruce Wills and see what happens ;-)
    if distance < 0.6 then
    begin
      Memo1.Lines.Add( 'Face[0] and Face['+IntToStr(i)+'] is very similiar ' );
      SetLength(similiar_faces, Length(similiar_faces)+1);
